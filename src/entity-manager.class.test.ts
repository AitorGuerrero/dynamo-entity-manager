...
describe('Having entity manager with transactional flusher', () => {
	...
	const keySchema = [
		{ KeyType: 'HASH', AttributeName: hashAttr },
		{ KeyType: 'RANGE', AttributeName: rangeAttr },
	];
		...
		describe('and deleting the entity', () => {
			...
			describe('and flushing', () => {
				...
				it('should not persist the entity', () => {
					expect(
						documentClient.getByKey(tableName, { [hashAttr]: hashAttr, [rangeAttr]: rangeAttr }),
					).to.be.undefined;
				});
			});
		});
	...
	describe('and Dynamo TransactWrite fails', () => {
		...
		it('should resolve promise to error', () =>
			expect(thrownError).to.be.equal(documentClientError));
	});
	describe('and there are more than 25 updates to be done', () => {
		...
		it('should emit error', () =>
			expect(emittedError).to.be.instanceOf(TransactionItemsLimitReached));
		it('should resolve promise to error', () =>
			expect(thrownError).to.be.instanceOf(TransactionItemsLimitReached));
	});
